...
export function getAllConfig() {
  return structuredClone(store.store); // 返回副本：调用方修改不污染持久化配置（旧实现返回内部引用）
}

// 统一读取 dsh 数据目录：环境变量 DSH_HOME 优先（dsh 自身的约定），其次用持久化配置
// （旧实现各处直接 getConfig('dshHome')，store 初始化时快照了 env，运行时改环境变量不会生效——优先级不一致）
export function getDshHome() {
  return process.env.DSH_HOME || getConfig('dshHome'); // env 优先，其次配置文件
}
...
export function getCordisPatchPath() {
  const dshHome = getDshHome(); // 统一经 helper 读数据目录（env 优先）
